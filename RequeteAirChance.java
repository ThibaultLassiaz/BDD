...
public class RequeteAirChance {
    ...
    public static void deleteVol(Connection conn) throws SQLException {
        ...
        // Execute the query
        ResultSet rs = stmt.executeQuery("select numvolpassager, datevol from volpassager");
        while(rs.next()){
            listvol.add(new VolPassager(rs.getInt(1), rs.getDate(2)));
        }

        for(int i =0; i < listvol.size(); i++) {

            System.out.println( i + "- Numero VolPassager : "+ listvol.get(i).getNumVolPassager() + " Date de vol : " + listvol.get(i).getDate());
        }

        int numVol = LectureClavier.lireEntier("Choisir le Vol a supprimer");

        List<String> listquery = vp.delete(listvol.get(numVol).getNumVolPassager(), listvol.get(numVol).getDate());

        for(int i=0; i < listquery.size(); i++){
            rs = stmt.executeQuery(listquery.get(i));
        }
    }
